...
##                        
   ...
   #                     df_temp = pd.merge(offense_team, defense_team, play_type, pass_type, target_name, cmpt_flag)
                        #print(df_temp)
...
temp_df = df_full[['offense_team', 'pass_type', 'target_name']]
temp_df.groupby(['offense_team', 'pass_type']).agg(lambda x:x.value_counts().index[0])
