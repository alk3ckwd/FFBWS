...
if refresh == 'Y':
    ...
    schedule_url = 'https://api.sportsdatallc.org/nfl-t1/2015/REG/schedule.json?api_key={key}'.format(key=key2)
    response = requests.get(url=schedule_url)
    sched_data = json.loads(response.text)
    with open('/users/mmcvicar/Documents/FFBAWS/2015_schedule.txt', 'w') as output:
    ...
    for week in sched_data['weeks']:
        for game in week['games']:
            ...
            if not os.path.isfile('/users/mmcvicar/Documents/FFBAWS/pbp_data/gamepbp_{game}.txt'.format(game=game['id'])):
                if game['status'] == 'closed':
                    base_url = 'https://api.sportsdatallc.org/nfl-t1/2015/{week}/{away}/{home}/pbp.json?api_key={key}'.format(week=week['number'], away=game['away'], home=game['home'], key=key2)
                    
                    r = requests.get(base_url)
                    data = json.loads(r.text)


                    with open('/users/mmcvicar/Documents/FFBAWS/pbp_data/gamepbp_{game}.txt'.format(game=game['id']), 'w') as output:
